  ...
  const handleRecoverSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok || data.ok || data.success) {
        setNote(data.message || 'Verification sent. If a matching purchase email exists, recovery info will arrive shortly.');
        setEmail('');
      } else {
        setNote(data.error || data.message || 'Registered email address not found in license database.');
      }
    } catch (err) {
      setNote('Connection failed. Please check your network and try again.');
    }
  };
